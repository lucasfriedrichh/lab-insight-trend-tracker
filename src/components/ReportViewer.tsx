...
import { ArrowLeft, Download, Calendar, User, FlaskConical, Settings, TrendingUp } from 'lucide-react';
...
export const ReportViewer: React.FC<ReportViewerProps> = ({ method, assay, onBack }) => {
          ...
          <html>
            <head>
              ...
              <style>
                ...
                .tool-section { margin-bottom: 30px; page-break-inside: avoid; }
  ...
  const renderToolData = (tool: Tool) => {
    return (
      <div key={tool.id} className="tool-section">
        ...
        <div className="bg-slate-50 rounded-lg p-4">
          <table className="data-table w-full">
            <thead>
              <tr>
                <th className="text-left font-medium text-slate-700">Parâmetro</th>
                <th className="text-left font-medium text-slate-700">Valor</th>
              </tr>
            </thead>
            <tbody>
              {Object.entries(tool.data).map(([key, value]) => (
                <tr key={key}>
                  ...
                  <td className="py-2 font-medium text-slate-800">{String(value)}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <div id="report-content" className="space-y-6">
        ...
        <Card className="shadow-lg">
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <Settings className="w-5 h-5 text-primary" />
              Ferramentas e Dados de Análise
            </CardTitle>
            <CardDescription>
              Equipamentos utilizados e seus parâmetros de operação
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-6">
            {assay.tools.map(renderToolData)}
          </CardContent>
        </Card>
